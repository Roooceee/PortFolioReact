...
export const Skills = ({sectionId}) => {
  ...
  return (
    <section id={sectionId} className="section background-secondary">
      <motion.div
        ...
        {!isReady && !error && <Loading textLoading={'Chargement des compétences en cours'} />}
        {isReady && !error && (
          <div className="flex flex-col gap-5 md:grid md:grid-cols-2 xl:flex xl:flex-row mx-auto">
            {skills.map((element) => {
              return (
                <Skill key={element.title} logo={element.logo} title={element.title} list={element.skills_details} />
              );
            })}
          </div>
        )}
