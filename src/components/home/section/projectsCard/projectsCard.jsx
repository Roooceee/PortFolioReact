...
export const ProjectsCards = ({sectionId}) => {
      ...
      if (result) {
        const projectsWithLanuages = await Promise.all(
          result.map(async (project) => {
            const languagesRes = await getDatas(
              `https://api.github.com/repos/Roooceee/${project.name}/languages`,
              token,
            );
  ...
  return (
    <section id="mes-derniers-projets" className="section background-primary">
      <div className="contain-1440 margin-auto flex flex-col justify-between flex-wrap min-h-[760px] gap-8 lg:px-8">
        ...
        {!isReady && !error && <Loading textLoading={'Chargement des projets en cours'} />}
        ...
        <motion.div
          ...
          <Link to="/tous-mes-projets" className="mx-auto button-blue" title="Voir tous mes projets">
